...
async function main() {
  ...
  console.log('🔒 Preserving protected accounts (***REDACTED-TEST-ORGANIZER-EMAIL***, ***REDACTED-ADMIN-EMAIL***)...');
  const preservedUsers = await prisma.user.findMany({
    where: { email: { in: ['***REDACTED-TEST-ORGANIZER-EMAIL***', '***REDACTED-ADMIN-EMAIL***'] } },
  });
