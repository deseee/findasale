...
export const getStaticProps: GetStaticProps<CityCategoryPageProps> = async ({ params }) => {
  ...
  try {
    ...
    console.error(`[city/category] fetch threw for ${citySlug}/${categorySlug}:`, err);
    throw err;
  }

  if (!res.ok) {
    throw new Error(`[city/category] by-city returned ${res.status} for ${citySlug}/${categorySlug}`);
  }
  ...
  if (data.geocoderStatus === 'error') {
    throw new Error(`[city/category] geocoderStatus=error for ${citySlug}/${categorySlug}`);
  }
