/**
 * Yard Sales City SEO Landing Page
 * URL: /yard-sales/denver-co
 *      /yard-sales/chicago-il
 *      /yard-sales/grand-rapids-mi
 *
 * Dedicated landing page for the "yard sales in [city]" / "garage sales in [city]" GSC cluster.
 ...
 *
 * Modeled after /estate-sales/[city-slug].tsx — same ISR pattern, same components,
 * yard-sale-specific copy and FAQ content.
 */
...
interface YardSalesCityPageProps {
  citySlug: string;
  cityName: string;
  cityState: string;
  cityMeta: CityMeta;
...
export default function YardSalesCityPage({
  ...
  const canonicalUrl = `https://finda.sale/yard-sales/${citySlug}`;
  ...
  const breadcrumbJsonLd = {
    ...
    itemListElement: [
      ...
      { '@type': 'ListItem', position: 2, name: 'Yard Sales', item: 'https://finda.sale/city/yard-sales' },
      {
        '@type': 'ListItem',
        position: 3,
        name: `Yard Sales in ${cityName}, ${cityState}`,
        item: canonicalUrl,
      },
    ],
  };
  ...
  return (
    <>
      <Head>
        ...
        <meta
          name="keywords"
          content={`yard sales ${cityName} ${cityState}, garage sales ${cityName}, ${cityName} yard sales, ${cityName} garage sales this weekend, yard sale finder ${cityName}`}
        />
      ...
      <main className="min-h-screen bg-white dark:bg-slate-900">
        ...
        <nav className="max-w-5xl mx-auto px-4 pt-4 pb-2 text-sm text-warm-500 dark:text-warm-400">
          <ol className="flex flex-wrap items-center gap-1">
            ...
            <li>
              <Link href={`/city/${citySlug}/yard-sales`} className="hover:text-amber-600">
                Yard Sales
              </Link>
            </li>
            <li aria-hidden="true">/</li>
        ...
        <div className="max-w-5xl mx-auto px-4 pt-6 pb-4">
          <h1 className="text-3xl md:text-4xl font-bold text-warm-900 dark:text-warm-100">
            Yard Sales in {cityName}, {cityState}
          </h1>
        ...
        <div className="max-w-5xl mx-auto px-4 pb-16">
              ...
              {nearbyLinks.length > 0 && (
                <div className="max-w-sm mx-auto">
                  <p className="text-warm-500 dark:text-warm-400 text-sm font-medium mb-3">
                    Yard sales near {cityName}:
                  </p>
                  <div className="flex flex-wrap gap-2 justify-center">
                    {nearbyLinks.map(({ slug, label }) => (
                      <Link
                        key={slug}
                        href={`/yard-sales/${slug}`}
                        className="px-3 py-1.5 rounded-full text-sm border border-warm-300 dark:border-gray-600 text-warm-700 dark:text-warm-300 hover:border-amber-500 hover:text-amber-600 transition-colors"
                      >
                        {label}
                      </Link>
          ...
          <div className="mt-8 p-6 bg-amber-50 dark:bg-amber-900/20 rounded-xl text-center border border-amber-200 dark:border-amber-800">
            <h2 className="text-lg font-semibold text-warm-900 dark:text-warm-100 mb-2">
              Running a yard sale in {cityName}?
            </h2>
            <p className="text-warm-600 dark:text-warm-400 text-sm mb-4">
              List your sale on FindA.Sale and reach local shoppers looking for yard sales right now.
            </p>
            <Link
              href="/register"
...
export const getStaticPaths: GetStaticPaths = async () => {
  let slugs: string[] = TOP_YARD_SALE_CITIES;
  ...
  try {
    ...
    if (res.ok) {
      ...
      if (fetched.length > 0) {
        ...
        const merged = Array.from(new Set([...TOP_YARD_SALE_CITIES, ...fetched]));
...
export const getStaticProps: GetStaticProps<YardSalesCityPageProps> = async ({ params }) => {
  ...
  try {
    res = await fetch(
      `${apiBaseUrl}/sales/by-city/${encodeURIComponent(citySlug)}?category=yard-sales`,
    ...
    console.error(`[yard-sales/[city-slug]] getStaticProps fetch threw for ${citySlug}:`, err);
    throw err;
  }

  if (!res.ok) {
    throw new Error(`[yard-sales/[city-slug]] by-city returned ${res.status} for ${citySlug}`);
  }
  ...
  if (data.geocoderStatus === 'error') {
    throw new Error(`[yard-sales/[city-slug]] geocoderStatus=error for ${citySlug}`);
  }
