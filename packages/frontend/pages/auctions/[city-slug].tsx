...
export const getStaticProps: GetStaticProps<AuctionsCityPageProps> = async ({ params }) => {
  ...
  try {
    ...
    console.error(`[auctions/[city-slug]] getStaticProps fetch threw for ${citySlug}:`, err);
    throw err;
  }

  if (!res.ok) {
    throw new Error(`[auctions/[city-slug]] by-city returned ${res.status} for ${citySlug}`);
  }
  ...
  if (data.geocoderStatus === 'error') {
    throw new Error(`[auctions/[city-slug]] geocoderStatus=error for ${citySlug}`);
  }
