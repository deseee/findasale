...
const HomePage = () => {
  ...
  const filteredSales = useMemo(() => {
    ...
    if (dateFilter !== 'all') {
      ...
      // Weekend = Saturday (6) or Sunday (0) of the current week
      const day = now.getDay(); // 0=Sun … 6=Sat
      const satOffset = (6 - day + 7) % 7;
      const sunOffset = (7 - day) % 7;
      const thisSat = new Date(todayStart); thisSat.setDate(thisSat.getDate() + satOffset);
      const thisSun = new Date(todayStart); thisSun.setDate(thisSun.getDate() + sunOffset);
      const weekendStart = new Date(Math.min(thisSat.getTime(), thisSun.getTime()));
      const weekendEnd = new Date(Math.max(thisSat.getTime(), thisSun.getTime()));
                ...
                <div key={sale.id} className="bg-white rounded-lg shadow-md overflow-hidden hover:shadow-lg transition-shadow">
                  <Link href={`/sales/${sale.id}`} className="block">
                    {sale.photoUrls && sale.photoUrls.length > 0 ? (
                      <img src={sale.photoUrls[0]} alt={sale.title} className="w-full h-48 object-cover" />
                    ) : (
