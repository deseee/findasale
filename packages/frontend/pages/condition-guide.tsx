...
const ConditionGuide: React.FC = () => {
  ...
  return (
    <Layout>
      ...
      <main className="bg-warm-50 py-8 md:py-12">
        <div className="container mx-auto px-4">
          {/* Header */}
          <div className="text-center mb-12">
            <h1 className="text-4xl md:text-5xl font-bold text-warm-900 mb-3">Item Condition Guide</h1>
            <p className="text-lg text-warm-600">Understanding FindA.Sale condition ratings</p>
          </div>
              ...
              return (
                ...
                >
                  ...
                  <div className="p-6 flex flex-col flex-1">
                    ...
                    {/* Name */}
                    <h2 className="text-xl font-bold text-warm-900 text-center mb-2">{condition.name}</h2>

                    {/* Description */}
                    <p className="text-sm text-warm-700 mb-4 flex-1">{condition.description}</p>

                    {/* Examples */}
                    <div className="mb-4">
                      <h3 className="text-xs font-semibold text-warm-600 uppercase mb-2">Common Examples:</h3>
                    ...
                    {/* Price range */}
                    <div className="bg-warm-50 rounded px-3 py-2 text-center">
                      <p className="text-xs text-warm-600">Typical Price Range</p>
                      <p className="text-sm font-semibold text-warm-900">{condition.priceRange}</p>
                    </div>
                  </div>
                </div>
              );
          ...
          <div className="bg-white rounded-lg shadow-md overflow-hidden mb-12">
            <div className="bg-gradient-to-r from-amber-50 to-warm-50 px-6 md:px-8 py-8">
              <h2 className="text-3xl font-bold text-warm-900 mb-2">Frequently Asked Questions</h2>
              <p className="text-warm-600">Common questions about item conditions</p>
            </div>
            ...
            <div className="divide-y divide-warm-200">
              {faqs.map((faq, idx) => (
                <details
                  key={idx}
                  className="group px-6 md:px-8 py-4 hover:bg-warm-50 transition-colors cursor-pointer"
                >
                  <summary className="font-semibold text-warm-900 flex items-center justify-between text-base">
                    {faq.question}
                  ...
                  <p className="text-warm-700 mt-4">{faq.answer}</p>
                </details>
              ))}
            </div>
          </div>
          ...
          <div className="bg-gradient-to-r from-amber-50 to-warm-50 rounded-lg shadow-md p-8 text-center">
            <h3 className="text-2xl font-bold text-warm-900 mb-2">Ready to browse items?</h3>
            ...
            <div className="flex flex-col sm:flex-row gap-4 justify-center">
              ...
              <Link
                href="/"
                className="inline-block bg-white hover:bg-warm-50 text-warm-900 border border-warm-300 font-semibold py-3 px-6 rounded-lg transition-colors"
              >
                Back to Home
              </Link>
            </div>
