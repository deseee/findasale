/**
 * City SEO Landing Page — SSR long-tail sibling (Option B)
 * URL: /city/:slug (served here only for cities NOT in the curated ISR
 *      top-N list — see middleware.ts's CURATED_CITY_SLUGS).
 *
 ...
 *
 * KEEP IN SYNC with pages/city/[slug].tsx (the curated ISR variant) --
 * the component/markup below is intentionally a duplicate of that file's
 * JSX so a bug here can never affect the higher-traffic curated cities,
 * and vice versa. If you change the rendered UI in one, change it in both.
 *
 * Reached ONLY via middleware.ts's NextResponse.rewrite() -- the public URL
 * a browser/crawler sees is always /city/:slug, never this file's own path,
 * so this does not create a duplicate-content or duplicate-URL issue.
 */
          ...
          {liveFaqs.length > 0 && (
            <div className="mt-6">
              <h2 className="text-lg font-semibold text-warm-900 dark:text-warm-100 mb-4">
                Frequently Asked Questions About Sales in {cityName}
              </h2>
...
export const getServerSideProps: GetServerSideProps<CityPageProps> = async ({ params, res }) => {
  ...
  try {
    ...
    console.error(`[internal-ssr/city/[slug]] city-slugs fetch error for ${citySlug}:`, err);
  }
