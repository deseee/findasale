...
const OrganizerSettingsPage = () => {
  ...
  // Universal Crosslister -- Discogs (Official-API Tier, Personal Access Token model,
  // mirrors the Reverb pattern directly above). Local-only, never logged/persisted
  // client-side beyond this input -- POSTed directly to /discogs/connect on submit.
  const [discogsTokenInput, setDiscogsTokenInput] = useState('');
  ...
  // Discogs connection status query (Universal Crosslister, Official-API Tier)
  const { data: discogsStatus, isLoading: discogsStatusLoading, refetch: refetchDiscogsStatus } = useQuery({
    queryKey: ['discogs-connection-status'],
    queryFn: () => api.get('/discogs/connection').then(r => r.data),
    enabled: !!user
  });

  // Connect Discogs mutation -- organizer pastes their own Personal Access Token
  const connectDiscogsMutation = useMutation({
    mutationFn: (personalAccessToken: string) => api.post('/discogs/connect', { personalAccessToken }),
    onSuccess: () => {
      setDiscogsTokenInput('');
      setIsConnectingDiscogs(false);
      refetchDiscogsStatus();
      showToast('Discogs account connected', 'success');
    },
    onError: (error: any) => {
      setIsConnectingDiscogs(false);
      const msg = error.response?.data?.message || 'Failed to connect Discogs account';
      showToast(msg, 'error');
    }
  });

  // Disconnect Discogs mutation
  const disconnectDiscogsMutation = useMutation({
    mutationFn: () => api.delete('/discogs/connection'),
    onSuccess: () => {
      refetchDiscogsStatus();
      showToast('Discogs account disconnected', 'success');
    },
    onError: (error: any) => {
      const msg = error.response?.data?.message || 'Failed to disconnect Discogs account';
      showToast(msg, 'error');
    }
  });
        ...
        <div className="max-w-4xl mx-auto px-4 py-8">
          ...
          {activeTab === 'discogs' && (
            <div className="space-y-6">
              <div className="card p-6">
                <div className="flex items-center gap-2 mb-4">
                  <h2 className="text-xl font-semibold text-warm-900 dark:text-gray-100">Discogs Account</h2>
                  <Tooltip content="Connect your Discogs account to push matching record and CD listings from FindA.Sale directly to Discogs." position="right" />
                </div>
                <p className="text-warm-600 dark:text-gray-400 mb-2">
                  Discogs connects with a Personal Access Token instead of a login popup. Generate one from your own Discogs account, then paste it below.
                </p>
                <ol className="text-sm text-warm-600 dark:text-gray-400 mb-6 list-decimal list-inside space-y-1">
                ...
                {discogsStatusLoading ? (
                  ...
                  <div className="space-y-4">
                    <div className="p-4 rounded-lg bg-green-50 dark:bg-green-900/20 border border-green-200 dark:border-green-800">
                      <div className="flex items-center gap-2 mb-2">
                        ...
                        <p className="font-semibold text-green-800 dark:text-green-200">Discogs Connected</p>
                      </div>
                      {discogsStatus?.externalUserId && (
                        <p className="text-sm text-green-700 dark:text-green-300">
                          Account: <span className="font-medium">{discogsStatus.externalUserId}</span>
                        </p>
                      )}
                      {discogsStatus?.connectedAt && (
                        <p className="text-sm text-green-700 dark:text-green-300">
                          Connected on {new Date(discogsStatus.connectedAt).toLocaleDateString()}
                        </p>
                      )}
                      {discogsStatus?.error && (
                        <div className="mt-3 p-2 rounded bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800">
                          <p className="text-xs text-red-700 dark:text-red-300">
                            <strong>Token issue:</strong> {discogsStatus.error}
                          </p>
                        </div>
                      )}
                    </div>
                    <button
                      onClick={() => disconnectDiscogsMutation.mutate()}
                      disabled={disconnectDiscogsMutation.isPending}
                      className="bg-warm-100 dark:bg-gray-700 hover:bg-warm-200 dark:hover:bg-gray-600 text-warm-900 dark:text-gray-100 font-semibold py-2 px-4 rounded-lg disabled:opacity-50 text-sm"
                    >
                  ...
                  <div className="flex flex-col sm:flex-row gap-3">
                    ...
                    <button
                      onClick={() => {
                        if (!discogsTokenInput.trim()) {
                          showToast('Paste your Discogs Personal Access Token first', 'error');
                          return;
                        }
                        setIsConnectingDiscogs(true);
                        connectDiscogsMutation.mutate(discogsTokenInput.trim());
                      }}
