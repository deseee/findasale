          ...
          <div className="bg-white rounded-lg shadow-xl p-6 max-w-md w-full">
            ...
            <div className="text-center mb-6">
              ...
              <div className="border-2 border-gray-300 rounded-lg p-4 inline-block">
                <img src={qrCodeUrl} alt="QR Code" className="w-48 h-48 mx-auto" loading="lazy" />
              </div>
                        ...
                        {analytics.sales.map((s) => (
                          <div key={s.id} className="bg-gray-50 rounded-lg p-4">
                            <div className="flex items-start justify-between mb-2">
                              <Link href={`/sales/${s.id}`} className="text-sm font-semibold text-gray-900 hover:text-blue-600">{s.title}</Link>
                              <StatusBadge status={s.status} />
                            </div>
                            <div className="grid grid-cols-2 gap-x-4 gap-y-1 text-xs text-gray-600">
                              <span>Sold: <span className="font-medium text-gray-800">{s.itemsSold}</span></span>
                              <span>Unsold: <span className="font-medium">{s.itemsUnsold}</span></span>
                              <span>Revenue: <span className="font-semibold text-green-700">${s.revenue.toFixed(2)}</span></span>
                              <span>Fees: <span className="text-orange-500">${s.fees.toFixed(2)}</span></span>
                              <span>QR Scans: <span className="text-purple-600 font-medium">{s.qrScanCount ?? 0}</span></span>
                            </div>
                          </div>
                        ))}
                        <div className="bg-gray-100 rounded-lg p-4 text-xs font-bold text-gray-700">
                          <div className="grid grid-cols-2 gap-x-4 gap-y-1">
                            <span>Total Sold: {analytics.itemsSold}</span>
                            <span>Unsold: {analytics.itemsUnsold}</span>
                            <span>Revenue: <span className="text-green-700">${analytics.totalRevenue.toFixed(2)}</span></span>
                            <span>Fees: <span className="text-orange-500">${analytics.totalFees.toFixed(2)}</span></span>
                          </div>
                        </div>
                      </div>
                      {/* Desktop table view */}
                  ...
                  <>
                    ...
                    <div className="md:hidden space-y-2">
                      {lineEntries.map((entry) => (
                        <div key={entry.id} className={`rounded-lg border p-3 flex items-center justify-between ${entry.status === 'CALLED' ? 'bg-yellow-50 border-yellow-200' : 'bg-white border-gray-200'}`}>
                          <div>
                            <span className="text-xs font-bold text-gray-500 mr-2">#{entry.position}</span>
                            <span className="text-sm font-medium text-gray-900">{entry.user.name}</span>
                            {entry.user.phone && <div className="text-xs text-gray-500 mt-0.5">{entry.user.phone}</div>}
                          </div>
                    ...
                    {sales?.map((sale: Sale) => (
                      <div key={sale.id} className="border border-gray-200 rounded-lg p-4">
                        <div className="flex items-start justify-between mb-1">
                          <div>
                            <div className="text-sm font-semibold text-gray-900">{sale.title}</div>
                            <div className="text-xs text-gray-500">{sale.city}, {sale.state}</div>
                          </div>
                          <StatusBadge status={sale.status} />
                        </div>
                        <div className="text-xs text-gray-500 mb-3">
                          {new Date(sale.startDate).toLocaleDateString()} – {new Date(sale.endDate).toLocaleDateString()} · {sale.items?.length || 0} items
                        </div>
                        <div className="flex flex-wrap gap-2 text-xs font-medium">
                          {sale.status === 'DRAFT' && (
                            <button onClick={() => handleStatusChange(sale.id, 'PUBLISHED')} disabled={statusLoading === sale.id}
                              className="bg-green-600 hover:bg-green-700 text-white font-bold py-1 px-3 rounded disabled:opacity-50">
                              {statusLoading === sale.id ? '...' : 'Publish'}
                            </button>
                          )}
                          {sale.status === 'PUBLISHED' && (
                            <button onClick={() => handleStatusChange(sale.id, 'ENDED')} disabled={statusLoading === sale.id}
                              className="bg-gray-600 hover:bg-gray-700 text-white font-bold py-1 px-3 rounded disabled:opacity-50">
                              {statusLoading === sale.id ? '...' : 'End Sale'}
                            </button>
                          )}
                          <Link href={`/organizer/add-items?saleId=${sale.id}`} className="text-blue-600 hover:text-blue-900">Add Items</Link>
                          <Link href={`/sales/${sale.id}`} className="text-green-600 hover:text-green-900">View</Link>
                          <button onClick={() => handleGenerateQR(sale)} className="text-purple-600 hover:text-purple-900">QR Code</button>
                        </div>
                      </div>
                    ))}
                  </div>
