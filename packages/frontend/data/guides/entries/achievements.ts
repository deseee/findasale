import { GuideEntry } from '../index';

const entry: GuideEntry = {
  slug: 'achievements',
  title: "Achievements and the Hall of Fame",
  audience: 'shopper',
  format: 'written',
  priority: 3,
  relatedGuides: ['how-ranks-work', 'streak-rewards', 'loot-legend'],
  videoUrl: undefined,
  body: `# Achievements and the Hall of Fame

Achievements are milestone badges you unlock for specific actions. The Hall of Fame is a public board that recognizes top XP earners each month. They're connected but different — here's how both work.

---

## What achievements are

An achievement is a one-time badge you earn by hitting a specific milestone. Things like making your first purchase, posting your first haul, visiting ten different sales, or completing your first trade.

Each achievement has a name, an icon, and an XP award. You earn the badge and the XP at the same moment. Some are simple (you'll hit them naturally). Some take real effort.

A few examples of the kinds of actions that trigger achievements:

- First purchase
- First haul post
- First review written
- Tenth sale visited
- Fifth organizer followed
- First referral who completes a purchase
- First trade completed
- First achievement earned (yes, that's a real one)

The full list lives in [/shopper/achievements](/shopper/achievements). You'll see which ones you've unlocked and which ones are still locked, along with the progress bars for ones you're close to.

---

## Achievements vs. rank

These are not the same thing and they don't control each other.

**Rank** is a continuous measure. It goes up as your lifetime XP grows. You can't have a high rank with no XP — they're the same thing.

**Achievements** are discrete unlocks. You can have a high rank with very few achievements (if you earned your XP through actions that don't hit achievement thresholds) or you can have many achievements with a lower rank (if you spread your activity across many different action types without stacking XP in one area).

Neither is better than the other. They show different things about your activity history.

---

## Where to see your achievements

Go to [/shopper/achievements](/shopper/achievements). You'll see a grid of all available achievements. Unlocked ones are filled in with your unlock date. Locked ones are grayed out with the condition shown so you know what it takes.

Your achievements are also visible on your public profile. Other shoppers can see your badges when they visit your profile page.

---

## The Hall of Fame

The Hall of Fame is at [/shopper/hall-of-fame](/shopper/hall-of-fame). It's a public board showing the top XP earners for the current month and all time.

**Monthly board:** The top 10 XP earners for the calendar month. This resets on the first of each month. The previous month's results are archived and stay visible.

**All-time board:** The top earners since the platform launched. Much harder to crack — these are people who have been hunting consistently for months or years.

---

## How to get on the Hall of Fame

Earn the most XP in a calendar month. The top 10 are featured.

The actions that move you up fastest are also the ones that cost nothing: posting hauls, keeping your login streak, completing trades, writing reviews, and referring friends. Hunt Pass members earn bonus XP on these same actions, which helps if you're competing for a spot.

If you land in the top 10, you earn a bonus XP award on top of whatever you accumulated that month. Check the [Guild Primer](/shopper/guild-primer) for the current bonus amount — it can change.

---

## What being on the Hall of Fame gets you

- A public feature on the [/shopper/hall-of-fame](/shopper/hall-of-fame) page for that month
- A bonus XP award (see [Guild Primer](/shopper/guild-primer) for current amount)
- A Hall of Fame badge on your profile for that month
- Your entry preserved in the monthly archive permanently

Past Hall of Fame entries don't disappear. You can scroll back through every month and see who made it.

---

## Common questions

**Do I need Hunt Pass to earn achievements?**
No. All achievements are available to free shoppers. Hunt Pass speeds up XP gain, which may help you unlock certain XP-threshold achievements faster, but the actions themselves are all free.

**Can I unlock the same achievement twice?**
No. Each achievement is a one-time unlock. The XP is awarded once. After that it's a badge on your profile.

**Are there secret achievements?**
A few. They won't appear in your locked list — you'll just get a notification when you hit the trigger. No hints here, but they tend to reward unusual combinations of activity.

**What if I'm close to a Hall of Fame spot but miss the cutoff?**
Your XP from that month still counts toward your rank. Nothing is lost — you just didn't make the monthly top 10. Try again next month.

**Can crews earn Hall of Fame recognition?**
There's a separate crew leaderboard. The Hall of Fame at [/shopper/hall-of-fame](/shopper/hall-of-fame) features individual shoppers. Crew standings are tracked separately.

**Are achievement XP amounts listed somewhere?**
Yes. The [Guild Primer](/shopper/guild-primer) has current XP values for achievements alongside all other XP sources.

---

## Related guides

- [How ranks work (Initiate to Grandmaster)](/guides/how-ranks-work)
- [Streak rewards: daily logins and how to keep them going](/guides/streak-rewards)
- [The Loot Legend: your lifetime stats and finds](/guides/loot-legend)`,
};

export default entry;
