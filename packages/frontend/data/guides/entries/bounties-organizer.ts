import { GuideEntry } from '../index';

const entry: GuideEntry = {
  slug: 'bounties-organizer',
  title: "Use bounties to source items for buyers",
  audience: 'organizer',
  format: 'written',
  priority: 3,
  relatedGuides: ['pricing-an-item-suggested-price-and-your-override', 'the-review-queue-from-photo-to-live-listing', 'schedule-a-sale-and-pick-your-visibility-window'],
  videoUrl: undefined,
  body: `A bounty is a buyer's public request. They post what they're looking for and what they'll pay. You can fulfill it if you come across the item.

Example: a shopper posts "Looking for a Singer Featherweight sewing machine — willing to pay $150." You find one at an estate clearing. You claim the bounty. They're pre-committed to buy it at your next sale.

Bounties let you presell inventory before your sale opens and learn what buyers actually want before you source.

---

## How to see open bounties

Go to **Organizer → Bounties**.

You'll see a feed of open requests from shoppers in your area. Each bounty shows:
- What the buyer is looking for (description, sometimes photos of what they want)
- Their target price
- How long the bounty has been open
- Distance from your location

Bounties expire if they go unclaimed or if the buyer cancels.

---

## How to claim a bounty

1. Find a bounty you can fulfill — you either have the item or expect to find one.
2. Tap **I Found This**.
3. Enter the item details: photos, condition, your asking price.
4. Tap **Submit**.

FindA.Sale notifies the buyer that you've found a match. They review your item details.

If they confirm, you list the item in your next sale and they're committed to buy it. You'll see the bounty marked as claimed and the item flagged in your inventory as pre-committed.

If they don't confirm within 48 hours, the bounty reopens and you can list the item normally.

---

## What happens when multiple organizers claim the same bounty

Multiple organizers can attempt to fulfill the same bounty. The buyer reviews claims in the order they came in and confirms the first one that meets their needs. If your claim isn't chosen, the item is yours to list at full price — you're not locked out of anything.

First to have the item confirmed wins the pre-committed buyer.

---

## Why bother with bounties

**You know the item is sold before your sale opens.** No uncertainty. The buyer has committed.

**You learn what people want.** Browse the bounty feed before you go sourcing. If five shoppers are looking for vintage cast iron, you know what to prioritize at the auction this weekend.

**You build relationships with repeat buyers.** When you fulfill a bounty, that buyer knows you pay attention. They follow you. They come back.

---

## Common questions

**Do I have to honor the buyer's target price?**
No. You enter your asking price when you claim a bounty. If your price is higher than what they said they'd pay, they can decline. The target price is what they're aiming for — it's not a binding cap on you.

**What if I claim a bounty and then can't find the item?**
You can withdraw your claim before the buyer confirms. Go to the bounty and tap Withdraw. The bounty reopens for other organizers.

**Can I claim bounties for item types I don't usually sell?**
Yes. If you're running a yard sale or pop-up and spot something a buyer is looking for, you can claim it. Bounties aren't restricted by your sale type.

**Do bounties show shoppers my contact information?**
No. Communication goes through FindA.Sale. The buyer isn't given your phone or email.

**Can I post my own bounties as an organizer?**
No. Bounties are posted by shoppers. Organizers claim them. If you want to signal what you're looking for as a buyer yourself, you'd use a personal shopper account.

**What if the buyer backs out after confirming?**
That's rare, but it happens. If a buyer confirms and then cancels, the pre-committed flag is removed from your item and you list it normally. The buyer's account tracks their claim history — serial cancelers lose the ability to post bounties.

---

## Related guides

- [Pricing an item: suggested price and your override](pricing-an-item-suggested-price-and-your-override.md)
- [The review queue: from photo to live listing](the-review-queue-from-photo-to-live-listing.md)
- [Schedule a sale and pick your visibility window](schedule-a-sale-and-pick-your-visibility-window.md)`,
};

export default entry;
