      ...
      try {
        ...
        if (!response.ok) {
          if ((response.status === 502 || response.status === 503) && attempt < MAX_RETRIES - 1) {
            ...
            console.log(
              `[run-craigslist] (${completed}/${totalBatches}) Batch ${batch.num} HTTP ${response.status} — retrying in ${delayMs}ms (attempt ${attempt}/${MAX_RETRIES})`
            );
        ...
        if (attempt < MAX_RETRIES - 1) {
          ...
          const delayMs = Math.pow(2, attempt) * 1000;
          console.log(
            `[run-craigslist] (${completed}/${totalBatches}) Batch ${batch.num} network error — retrying in ${delayMs}ms`
          );
          await new Promise((r) => setTimeout(r, delayMs));
          continue;
        }
