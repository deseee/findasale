...
async function main() {
  ...
  // GitHub Actions console log instead of being silently swallowed. Mirrors the
  // 2026-07-21 fix applied to run-estatesalesnet.ts for the same 0-created/
  // high-failed regression pattern.
  ...
  // Diagnostic only — surfaces the reasons behind `failed` above, which were
  // previously visible only via direct Railway log/DB access (see 2026-07-21
  // health-check finding: 0 created / 196 failed with no visible cause).
  if (failureReasonTotals.size > 0) {
    const topReasons = Array.from(failureReasonTotals.entries())
      .sort((a, b) => b[1] - a[1])
      .slice(0, 10);
    console.log('[run-fb-events] Top item-level failure reasons:');
    for (const [reason, count] of topReasons) {
      console.log(`[run-fb-events]   ${count}x — ${reason}`);
    }
  }
}
