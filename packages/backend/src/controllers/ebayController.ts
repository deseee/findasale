...
import {
  ...
  // WeightTierMapping/CubicTierMapping were only used by the now-retired weight-tier-ladder
  // cascade match (resolvePoliciesForItem). detectWeightTierGaps/computeGapFillBuckets
  // followed them out with the gap-fill endpoints (2026-08-16 removal above). All are
  // still exported from ebayPolicyParser.ts -- only parseWeightTiers/classifyPolicy are
  // imported here, by getEbaySetupData's suggested-tier + classification display.
...
// (2026-08-16, Patrick-authorised removal) previewWeightTierGapFill /
// fillWeightTierGaps and their resolveOrganizerFromZip helper lived here. They
// provisioned real, permanent eBay fulfillment policies into the weight-tier ladder
// that ADR-102 (roadmap #622) retired from routing, so nothing prices from what they
// created. Their routes (/weight-tier-gaps/preview, /weight-tier-gaps/fill) and the
// settings-page panel that called them were removed in the same pass. The shipping-preset
// routes are the supported way for an organizer to create a policy they actually use.
