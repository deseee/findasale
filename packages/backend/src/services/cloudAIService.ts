...
Category: Pick the single best fit by the item's PRIMARY USE/DOMAIN — not its materials or whether it plugs in. A powered device is categorized by what it is FOR (e.g. an aquarium air pump is "Pet Supplies", a guitar amp is "Musical Instruments", a kitchen mixer is "Kitchenware"), NOT "Electronics". A single-issue comic book (not a bound trade paperback/collected graphic novel) is "Collectibles" (eBay's real category for individual comics), NOT "Books & Magazines" (that's for bound books/magazines). Choose from: ${EBAY_L1_CATEGORIES.join(', ')}.
...
async function getHaikuAnalysisMultiImage(
  ...
  const multiImagePrompt = imageCount > 1
    ? `You are analyzing ${imageCount} photos. These are usually different angles of the SAME single item — but they may instead show the multiple matching pieces of ONE set or lot sold together (for example, each photo a different club in a matched golf set, or different pieces of one dish set). Decide from the images which case applies before cataloging.`
