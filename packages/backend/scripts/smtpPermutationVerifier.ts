...
/**
 * SMTP Permutation Email Verifier
 *
 * For organizers with a known domain (website) but no contactEmail:
 *   1. Look up MX records for the domain
 *   2. Generate common email prefix patterns (info@, contact@, hello@, ...)
 *   3. Verify each via raw SMTP handshake (EHLO → MAIL FROM → RCPT TO)
 *      — never sends actual mail, just checks if the server accepts the address
 *   4. Detect catch-all domains (accept every address) to avoid false positives
 *   5. Write the first verified address to contactEmail
 *
 * Why this works: estate sale organizers often hide emails from HTML scrapers
 * but their mail server still accepts connections and responds to RCPT TO probes.
 *
 * Port 25 note: GitHub Actions (Azure) may block outbound port 25.
 * If you see 100% timeouts, run this script locally or via Railway CLI instead.
 *
 * Usage:
 *   DATABASE_URL=... npx ts-node scripts/smtpPermutationVerifier.ts
 *   SMTP_VERIFY=false DATABASE_URL=... npx ts-node scripts/smtpPermutationVerifier.ts
 *     (SMTP_VERIFY=false writes info@ as best-guess without verification)
 */
