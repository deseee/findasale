...
- [ ] **Push the 3 GitHub Actions workflow changes** (from prior session, sitting in your local git):
  ```
  git add .github/workflows/ci-typecheck.yml .github/workflows/geocode-ungeocoded-sales.yml .github/workflows/scrape-publicsurplus.yml
  git commit -m "ci: paths-ignore docs-only commits; geocode 3x->1x/day; PublicSurplus weekly->monthly"
  .\push.ps1
  ```
