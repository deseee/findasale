...
| 300 | ⬜ | ⬜ | ⬜ | ⬜ | ⬜ | ⬜ | Specced S495 — Build Pending | Return-to-Inventory Flow (Sale Close) | ORG | SIMPLE | Build after: saleType-aware prompt at sale close, Flip Report snapshot, price sync, eBay status check gate. Chrome QA: FLEA_MARKET sale close → unsold items offered for return → accept → items appear in inventory with updated price; verify eBay listing NOT touched (still live on eBay). | S495 spec: When a sale closes, unsold items offered for return based on saleType: FLEA_MARKET defaults return, ESTATE defaults archive, YARD/AUCTION prompts organizer. INTERNAL ONLY — no eBay listing teardown. The inventory source item (inInventory=true, has ebayListingId) is never touched; only the sale copy (inInventory=false, libraryId→source) is moved. Return flow: (1) snapshot Flip Report (bought price vs sale price vs eBay current), (2) resolve any PENDING holds first, (3) update sale copy price back to source if changed, (4) set sale copy inInventory=true OR delete copy and re-activate source. eBay listing continues independently — eBay sync webhook handles sell-through if it sells on eBay while sitting in inventory. Cloudinary photo migration note: long-term inventory items may need photo re-hosting from eBay CDN URLs. Schema change needed: add returnedToInventoryAt DateTime? on Item. |
