...
| 228 | ✅ | ✅ | ✅ | ✅ | ⬜ | ⬜ | ⬜ FIXED S531 — fee % double-multiply removed, pending Chrome QA | Settlement Hub (Commission Calc + Expense Tracker + Client Payout) | ORG | SIMPLE | Chrome QA: receipt step shows correct platform fee % (e.g. 2%, not 200%). Prior: wizard flow ✅ S515, expense tracking ✅, dark mode partial ✅. | 4 new Prisma models (SaleSettlement, SaleExpense, ClientPayout, SaleTransaction). 7 backend endpoints. SettlementWizard.tsx + 3 sub-components. /organizer/settlement/[saleId] page. S531: commissionRate is integer (8=8%); frontend was ×100 again. Fixed. |
...
| 309 | ⬜ | ⬜ | ⬜ | ⬜ | ⬜ | ⬜ | Planned S532 — Not started | Consignor Portal & Payouts | ORG | TEAMS | Chrome QA: Consignor invited via link → views their items + earnings. Organizer runs payout → consignor balance updates. Payout history shows correct split per consignment agreement. | S532 Spec: New Consignor model (workspaceId, name, email, phone, commissionRate, notes). New ConsignorPayout model (consignorId, saleId, totalSales, commissionAmount, netPayout, paidAt, method). Item.consignorId FK. consignorController.ts (CRUD + payout run). GET /consignors/:id/portal (public token-gated). consignors.tsx management page. ConsignorPayoutModal.tsx. Migration required. Architect spec: claude_docs/research/retail-mode-competitive-analysis-2026-04-21.md. Estimated scope: M (2 new models, 1 new page, 1 new modal, 4–6 endpoints). Closes competitive gap vs Ricochet/SimpleConsign. |
| 310 | ⬜ | ⬜ | ⬜ | ⬜ | ⬜ | ⬜ | Planned S532 — Not started | Color-tagged Discount Rules | ORG | TEAMS | Chrome QA: Organizer creates color-tag rule (e.g. Yellow = 25% off). Organizer assigns tagColor to item. Shopper views item — discounted price shown. Sale detail shows color-key legend. | S532 Spec: Item.tagColor String? (hex or named color). New DiscountRule model (workspaceId, tagColor, discountPercent, label, activeFrom, activeTo). discountRuleController.ts (CRUD). Price resolution: effectivePrice = tagColor && activeRule ? basePrice * (1 - rule.discountPercent/100) : basePrice. ColorTagRulesPage.tsx (/organizer/color-rules). TagColorPicker in item editor. ColorKeyLegend on sale detail. Migration required. Closes thrift-rack pricing gap — core feature for RETAIL saleType organizers. |
| 311 | ⬜ | ⬜ | ⬜ | ⬜ | ⬜ | ⬜ | Planned S532 — Not started | Multi-Location Inventory View | ORG | TEAMS | Chrome QA: TEAMS organizer creates 2+ locations. Inventory page has location filter. Item assigned to location — filter works. Sale created under a location — location label shows on sale card. | S532 Spec: New Location model (workspaceId, name, address, phone, managerId). Sale.locationId FK?. Item.locationId FK?. locationController.ts (CRUD). Inventory filter by locationId. LocationSelector on create-sale + item editor. LocationsPage.tsx (/organizer/locations). Cross-location item transfer endpoint. Migration required. Prerequisite for franchise/chain use case — unlocks multi-store operators as customer segment. |
