...
| 229 | ⬜ | ⬜ | ⬜ | ⬜ | ⬜ | ⬜ | Spec Complete S367 — Phase 2 | AI Comp Tool (Photo → eBay Sold Comps → Price Suggestion) | ORG | SIMPLE | Dispatch after Phase 1 ships | Photo upload → Google Vision → eBay sold listings search → price range suggestion. No competitor has this. Unique FindA.Sale differentiator. |
...
| 235 | ⬜ | ⬜ | ⬜ | ⬜ | ⬜ | ⬜ | Spec Complete S367 — Phase 2 | Charity Close + Tax Receipt PDF | ORG | SIMPLE | Phase 2 after Settlement Hub ships. | Estate/consignment only. Unsold items → charity. 4-step wizard: charity select, item review, pickup scheduling, PDF receipt generation. Schema: SaleDonation, TaxReceipt. Email receipt optional (not required). |
