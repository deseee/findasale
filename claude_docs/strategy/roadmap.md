...
| 303 | NA | NA | ✅ | NA | ⬜ | ⬜ | Shipped S513 — Pending Chrome QA | Photo Station Shopper Page | SHO | FREE | Chrome QA: navigate to /sales/[id]/photo-station as shopper, verify 5 XP auto-awarded via POST /sales/:saleId/photo-ops/photo-station-scan, verify "Already Scanned" state on re-visit, verify share button (Web Share + clipboard fallback) | S513: pages/sales/[id]/photo-station.tsx (225 lines). Auth-gated shopper page. QR-code landing point for organizer-placed Photo Op Station markers. Auto-awards 5 XP on load. Duplicate scan protection (re-visit shows "Already Scanned" state). Share incentive UI (no /xp/share endpoint yet — deferred). |
| 304 | NA | NA | ✅ | NA | ⬜ | ⬜ | Shipped S513 — Pending Chrome QA | Early Access Cache Items Page | SHO | FREE | Chrome QA: navigate to /shopper/early-access-cache/items, verify items grid loads from GET /api/early-access-cache/items, verify category badge/emoji + title (line-clamped) + price + "View Item" link | S513: pages/shopper/early-access-cache/items.tsx (199 lines). Auth-gated shopper page showing items from active early-access windows. Category emoji badges, line-clamped titles, price display, "View Item" deep link to /items/[id]. Empty state. Full dark mode. Replaces #291 Lucky Roll as the primary discovery/reward mechanic. |
