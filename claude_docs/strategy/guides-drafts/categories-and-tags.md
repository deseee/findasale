...
relatedGuides:
  ...
  - edit-live-listing
linkFrom:
  - Review queue item card (category and tag fields)
  - FAQ "Why isn't my item showing up in search?"
---

Every item in your sale has a category and a set of tags. Categories control which filter a shopper uses to find the item. Tags control whether the item shows up when a shopper types something specific into search. The app pre-fills both from photo recognition — your job in the review queue is to confirm they're right, correct the ones that aren't, and occasionally add a tag the app missed.

Voice notes are a second source of tag and category data. If you record a voice note during a rapidfire session — "mid-century walnut side table, good condition, small scratch on top" — the app reads those words and suggests a category (Furniture) and tags (mid-century-modern, walnut) based on what you said. Empty fields get filled in; fields you've already set are left alone. See [Photograph an entire sale in one pass](rapidfire-mode) for how to record voice notes during a session.
