---
title: "Built-in share cards: pick a theme and post it"
slug: share-cards
audience: organizer
format: W
priority: P3
treatment: FRESH
relatedGuides:
  - promote-page
  - brand-kit
  - send-sale-updates
linkFrom:
  - Promote page Print & Share section
  - FAQ
---

Share cards are pre-designed image files with your sale details already in them — sale name, dates, featured categories, and a link. Pick a theme, download the image or share it directly, and post it alongside your text in any Facebook group, on Instagram, or in a text message. Image posts get significantly more attention in local groups than text-only posts.

---

## What share cards are

A share card is a static image — like a flyer, but sized for social media. It looks designed even if you have never used a design tool. Your sale name, dates, and categories pull in automatically from your listing.

If you have a logo and accent color in your brand kit, those appear on the card. If not, the card uses a clean default style that still looks intentional.

Share cards are not the same as the pre-filled social media text on the promote page. The text and the image are separate. Use both together for the best result — image posts with a link in the caption outperform text-only posts and image-only posts.

---

## How to get your share cards

1. Go to /organizer/promote
2. Scroll to the **Print & Share** section
3. You will see 3–5 card themes — tap each one to preview it with your sale details
4. Pick the theme you want
5. Tap **Download** to save the image to your device, or tap **Share** to open your phone's share sheet and send directly

That is all. The image is ready to post.

---

## Choosing a theme

Themes are visual styles — different color palettes, layouts, and typography. All of them include the same information (your sale name, dates, categories). The differences are aesthetic.

A few tips:
- Pick the theme that matches your accent color if you have one set in your brand kit
- For estate and moving sales, darker or warmer themes tend to read as more serious
- For yard sales, flea market booths, and pop-up consignment, brighter or lighter themes work well
- When in doubt, pick the one you would stop scrolling to look at

---

## Where to use share cards

### Facebook local groups

Image posts in Facebook buy-sell and community groups stop the scroll. Text posts get skipped. Post the share card as the image, then add the text from the promote page as your caption, with your sale listing URL at the bottom.

**Format:**
1. Start a new post in the group
2. Attach the share card image
3. Paste your pre-filled Facebook text from the promote page as the caption
4. Add your FindA.Sale listing link at the end of the caption
5. Post

### Instagram Stories

Stories disappear after 24 hours, which makes them good for day-before or day-of announcements. Share card dimensions are sized to fit Stories.

**To post as a Story:**
1. Open Instagram → Your profile → Add to Story
2. Select the share card from your camera roll
3. Add a link sticker pointing to your FindA.Sale listing
4. Tap Share

### Instagram Feed

For the main feed, post the share card as a photo post. Write a short caption with your sale details and the listing link. Tag your location.

### Text message

Download the share card and attach it to a text message to regular customers, neighbors, or family. Most phones send images in MMS without any extra steps.

### Email

Attach the share card image to an email blast or paste it into the body of an email. The promote page also has a pre-formatted email you can open directly in your email app.

---

## Image posts vs. text posts: what the numbers say

In local Facebook groups, image posts receive substantially more engagement than text-only posts. The exact difference varies by group and timing, but the pattern is consistent: people scroll past text, people stop for images.

A share card paired with clear text and a link is the most effective single thing you can post to a local group. It is also the fastest — download, attach, paste caption, post.

---

## Common questions

**Do I have to use a share card, or can I just post text?**

Text posts work. Share cards work better, especially in visual platforms like Instagram and in Facebook groups where dozens of posts compete for attention. Use whichever you have time for, but if time allows, use both.

**Can I edit the card to add a specific item?**

Not directly inside FindA.Sale — the card generates from your listing details. If you want to customize further, download the image and open it in any basic photo editor or app (Canva, Google Photos, iPhone Photos) to add a text overlay.

**What size are the share cards?**

Cards are sized for social sharing — 1080x1080 pixels (square format) which works on Facebook, Instagram feed, and most messaging apps. Instagram Stories crop to 1080x1920, but square images post to Stories without issue by adding a background.

**Will the card update automatically if I change my listing?**

No. The card is a static image generated at the time you download it. If you make significant changes to your listing after downloading, return to the Print & Share section and download a fresh card.

**How is a share card different from a print card?**

Share cards are sized for screens and social media. The print card (from the promote page or print kit) is sized for paper — 4x6 or half-page, higher resolution, formatted for physical printing. Use share cards for posting online; use print cards for handing out or posting on bulletin boards.

**Can I use the same share card for multiple sales?**

No. Each card is generated for a specific sale listing and includes that sale's name and dates. Generate a new card for each sale.

---

## Related guides

- [The promote page: share your sale on 8 platforms](promote-page.md)
- [Set up your brand kit (logo, colors, and storefront URL)](brand-kit.md)
- [Send sale updates to followers (and what to write)](send-sale-updates.md)
