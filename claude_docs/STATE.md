...
**Latest work (S545 — IN PROGRESS):** P0 auth crash fixed. Railway was throwing `PrismaClientKnownRequestError: The column User.tasteProfile does not exist` (P2022) on every authenticated request — `authenticate` middleware crashing platform-wide. Root cause: S544 migration applied to Railway with snake_case `taste_profile` column name, but Prisma schema has camelCase `tasteProfile` with no `@map`. Created rename migration `20260422220500_rename_taste_profile_camelcase` (idempotent DO block checking information_schema) — deployed via `prisma migrate deploy`. Verified via psycopg2: `User.tasteProfile` column exists camelCase, `api_keys` table present. P0 `/organizer/sales` auto-resolved (was auth-crash symptom — Patrick confirmed "working"). Parallel dispatches: (1) Architect decision on rank-based early access — recommends Option A (build `publishedAt` + rank-tier time windows: Scout 1h / Ranger 2h / Sage 4h / GM 6h), ~6–8h effort, 4 open questions for Patrick; (2) Dev — mobile organizer dashboard fix (dashboard.tsx lines 1145/1150, responsive flex classes); (3) Dev — Organizer Insights Decimal fix (insightsController.ts lines 222–274, `.toNumber()` conversion per S543 precedent). Two commits already on GitHub via MCP: cf9c7b39 (original migration SQL fix), ea885c37 (rename migration).
