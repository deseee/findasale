...
| Auction vertical — AuctionZip → FIX CODED (pending push, BEST ROI) | **P2** — **S890 FIX CODED + validated** (auctionZipScraper.ts parser rewrite, TS 0 errors): old BEM-class regex replaced with current markup (`/XX-Auctioneers/<id>.html` anchors, company-name preferred over contact). Validated via Chrome MCP against live page → **235 US auctioneers parsed** from letter A alone (~25k across A-Z). Note: VM egress IP is Cloudflare-blocked for auctionzip.com, but the Railway relay path is the production fetch path. | Push → run scraper → confirm >0 AuctionZip organizers in DB | S887 |
...
- **Ops follow-up (post-deploy, not code):** trigger AuctionNinja workflow_dispatch + read Railway logs; verify FB Marketplace proxy env vars live on Railway. Deferred to S891 (need deployed code + secrets).
