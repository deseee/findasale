...
| CI gate — FRONTEND typecheck blocking only | **S1027 (corrected).** Frontend typecheck now gates (csv-parse fixed via tsconfig `types` allowlist; CI run passed). Backend typecheck reverted to non-blocking — it has ~30 REAL pre-existing type errors vs the committed schema (CI run #17). Dockerfile kept `tsc \|\| true`. | **Dedicated dev cleanup (findasale-dev + schema review):** fix the ~30 backend type errors (`sale.userId`, `prisma.qrScannerEvent`→`qRScannerEvent`, `isUnmanagedListing` selects, `processed`/`enriched`, null-safety, Stripe `never`). Some are latent runtime bugs. THEN drop `continue-on-error` on backend typecheck + `\|\| true` in Dockerfile. Also: tests (`--forceExit`+infra) + lint (add eslint config) to gate those. | S1026 → S1027 |
