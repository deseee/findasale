...
**S543 priority queue:**
1. **Hunt Pass "What's included" review** — Navigate to https://finda.sale/shopper/hunt-pass. Audit the "What's included" section: cross-check every listed benefit against the canonical feature set (hunt-pass.tsx, guild-primer.tsx, xpService.ts). Flag missing benefits, stale copy, or benefits that no longer exist. Dispatch findasale-dev to fill gaps.
2. **Investigate print kit 500 (P0)** — `/api/items/drafts?saleId=cmnxvyic4001li51qobwidrbl` returns 500. Sale "Artifact Downtown Paw Paw" is RETAIL type, Apr 1–30 (whole month). Start: `grep -r "drafts" packages/backend/src/routes/` to find route, then read getDrafts in itemController.ts. Check if RETAIL type or month-long date range triggers unhandled code path.
3. **Fix #241 Brand Kit PDFs (P1)** — Change hrefs in `packages/frontend/pages/organizer/brand-kit.tsx` from `/api/brand-kit/organizer/...` (relative Vercel) to `${process.env.NEXT_PUBLIC_API_URL}/brand-kit/organizer/...` (Railway direct).
4. **Fix /coupons coupon Generate buttons (P1)** — No API call fires on click. Read `packages/frontend/pages/coupons.tsx` shopper Generate button onClick, trace to coupon generation endpoint, wire correctly.
5. **P2 fixes (batch):** ActionBar.tsx `/shopper/trails` → `/trails` (Treasure Trails public browse); "Hunt Pass Active" badge showing for Karen who has no Hunt Pass; /shopper/ranks rank badge/earned message mismatch at Scout boundary.
6. **Organizer Insights runtime error (P2)** — "Failed to load" on /organizer/insights. Pre-existing. Check Railway logs for the error trace.
7. **Continue Chrome QA backlog** — S540 Rewards nav links (4 locations), dashboard achievements dedup, orphan ref hops, S529 storefront widget, #267 RSVP XP, per-sale analytics, settlement fee %.
8. **S542 smoke test** — Check cart drawer opens/closes, holds + cart items show, Remove button works, price displays correctly, Explore dropdown opens with Feed/Calendar/Wishlist, dark mode toggle in AvatarDropdown.

**Patrick actions:**
- Push S542 changes (push block below)
