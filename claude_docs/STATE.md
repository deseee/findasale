...
| Auction vertical — AuctionZip → Cloudflare IP-block confirmed; CHROME HARVEST queued | **P2** — **S891: UA swap tested + RULED OUT.** Changed fetch to `getRandomUserAgent()` (matching AuctionNinja); deployed + re-run → STILL HTTP 403 on every letter page (A onward). Confirms a hard Cloudflare challenge / datacenter-IP block, NOT a UA heuristic. Free server-side path exhausted. **PLAN (Patrick's directive): one-time Chrome MCP harvest** — auctionzip.com loads fine via real browser/residential IP (S890 Chrome investigation parsed 235/page), and the directory is slow-changing so a single harvest of the 26 `/Auctioneer-Directory/[A-Z].html` pages (~25k auctioneers) suffices. Parser logic already in auctionZipScraper.ts; insert via getOrCreateScrapedOrganizer. (Recurring automation later = free CF-worker GET-forwarder experiment OR paid Cloudflare-solving API free tier.) | NEXT SESSION: Chrome MCP harvest of 26 letter pages → bulk insert. See Next Session dispatch stub. | S887 |
