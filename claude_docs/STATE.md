...
| CI gate — FRONTEND + BACKEND typecheck blocking | **RESOLVED S1027.** Both typecheck steps gate (continue-on-error removed); Dockerfile `npx tsc` (no `\|\| true`). All ~73 backend type errors fixed across 3 dev passes (verified via uncapped CI raw log + committed schema). CONFIRM the bundling push shows backend typecheck GREEN in CI — if a residual error appears, revert backend step to `continue-on-error: true` + Dockerfile `\|\| true` and dispatch the remainder. | Lower priority: backend TESTS (1 file, needs `--forceExit`+infra) + frontend LINT (no eslint config in repo) still non-blocking — add config to gate later. | S1026 → resolved S1027 |
