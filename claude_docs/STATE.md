...
**S809 complete:** Sentry triage (8 issues closed/ignored), 7 new DB indexes + Review.organizerId denorm + migration 20260530000001 deployed, MulterError Sentry filter shipped. Railway DB password confirmed active.
...
5. **Dead-widget triage:** ~17 built-but-hidden widgets — render or cut per widget. Decision needed before we proceed.
6. **#463 follow-up:** Confirm Google approves the ~52 products after 3-day Merchant Center review.

**Dispatch stubs for next session:**
- **[FIRST TASK] Verify S809 migration:** Connect to Railway DB via psycopg2, run `SELECT indexname FROM pg_indexes WHERE tablename IN ('Organizer','Sale','Review','DirectoryClaimEmail') AND indexname LIKE '%2026%' ORDER BY tablename;` — confirm all 7 new indexes + Review.organizerId column exist. If missing, re-run `prisma migrate deploy` with live DATABASE_URL.
- `Skill('findasale-dev')` → P2 unbounded findMany cleanup on public endpoints (citiesController + others). Root cause: public list endpoints call findMany with no take/cursor. Expected output: bounded queries (take + cursor/limit) + push block.
