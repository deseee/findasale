...
| Auction vertical — AuctionNinja ✅ WORKING (S890) | **RESOLVED** — Patrick ran the scraper S890; DB went **0 → 576 organizer records** (psycopg2 confirmed, directoryMostRecentSource ILIKE '%AuctionNinja%'). Relay path (scrape-auctionninja.yml → Railway endpoint → runs from Railway IP) works; Railway GCP IP is NOT Cloudflare-blocked. No further action — monitor weekly run. | Done — verify weekly cron keeps producing | S868 |
...
| Geocoding backlog → FIX DEPLOYED, NOT YET PROVEN | **P1** — root cause S890: of 15,792 un-geocoded, **14,628 (93%) are ENDED** (never mapped); only 1,164 live PUBLISHED (953 GSF + 211 FB Events). Fix (status='PUBLISHED' + oldest-first) is PUSHED + deployed (Patrick S890, green). **BUT post-deploy the live un-geocoded count is STILL exactly 1,164 (953/211 split unchanged)** — the geocode run that fired hit old code or pre-deploy schedule. NOT verified — count must actually drop. Needs a fresh geocode workflow_dispatch AFTER the backend redeploy, then re-check. FB Events 211 should geocode via city-center; GSF 953 may still ~80% fail Nominatim (structural). | Trigger geocode workflow_dispatch now → re-check `COUNT WHERE lat IS NULL AND status='PUBLISHED'` drops below 1,164 | S887 |
...
| Auction vertical — AuctionZip → PARSER FIXED, but Cloudflare-403 blocks the fetch | **P2** — **S890: parser rewrite deployed + correct** (Chrome-validated 235 auctioneers/page). BUT post-deploy run (S890, Railway logs 02:24-02:25 UTC Jun 6) returned **HTTP 403 on EVERY letter page A-V** — auctionzip.com is Cloudflare-protected and blocks Railway's IP before the parser ever sees HTML. (Contrast AuctionNinja: not Cloudflare-protected, works fine from Railway = 576 records.) The parser fix is sound; the blocker is now purely network/IP. Options: (a) generalize the FB Marketplace CF Worker into a generic GET forwarder + route AuctionZip through it — CHEAP but uncertain (FB evidence shows CF Worker edge IPs can also be soft-blocked by Cloudflare-protected targets); (b) paid residential proxy (ScraperAPI/BrightData, reliable vs Cloudflare); (c) headless browser w/ real fingerprint (the investigation reached the site via Chrome). DECISION needed. | Patrick decision: CF-worker experiment (cheap) vs paid proxy vs drop AuctionZip | S887 |
