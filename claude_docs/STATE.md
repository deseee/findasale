...
- **[DEV]** Scraper/enrichment fixes from audit. Dispatch `findasale-dev` for:
  ...
  - ~~Outreach OUTREACH_ENABLED=false leak~~ — RESOLVED S889 (propagation-lag false alarm; gate works across all send paths; 0 sends since 22:39 Jun 5 redeploy). No dev work needed.
...
2. #335 RESUME (when ready): (a) reactivate outreach@finda.sale at admin.google.com, (b) set `OUTREACH_ENABLED=true` in Railway, (c) re-enable the disabled "Pipeline — Outreach Emails" GitHub workflow (flag alone won't resume sending — workflow is the trigger). Backend caps at 1,500/day + per-window quota; keep warmup ~200/day. (S889 confirmed no leak — safe to resume on your schedule.)
