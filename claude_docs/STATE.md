...
| G1 Transactional Resend rail 403-REJECTED — whole rail down | **P0 — CONFIRMED ROOT CAUSE (Resend Log 6b2ba6a0, S937).** Railway `SES_FROM_EMAIL` = `find@outreach.finda.sale`. Resend error verbatim: "Domain not verified: Verify outreach.finda.sale or update your from domain." Resend has ONLY `finda.sale` (root) verified — `outreach.finda.sale` and `send.finda.sale` are NOT Resend domains. Since all 9 transactional callers pass `from: process.env.SES_FROM_EMAIL || …`, EVERY Resend transactional send (password reset, email verification, Stripe receipts/payouts, POS receipts, workspace invites, consignor/DM emails) AND the daily gmailHealthCron/deliverabilityMonitor alert sends are 403-rejected and NOT delivered. Only the admin send-test (RESEND_FROM_EMAIL=support@finda.sale, verified root) got a 200. COUPLING: `SES_FROM_EMAIL` is shared with the Gmail rail (where find@outreach.finda.sale IS a valid send-as alias) — that's why it's set to the outreach subdomain; can't be repointed without affecting Gmail From. FIX (recommend, awaiting Patrick OK on from-address): decouple the Resend rail from SES_FROM_EMAIL — coerce any non-`finda.sale` from to the verified `RESEND_FROM_EMAIL ?? noreply@finda.sale` default inside transactionalEmailService.send (ONE file, no auth/payment-controller edits; receipt From becomes noreply@finda.sale). Also set Railway RESEND_FROM_EMAIL=noreply@finda.sale. Red-flag QA review (payment email). | Patrick decision: approve the transactionalEmailService coercion fix + RESEND_FROM_EMAIL=noreply@finda.sale. | S937 |
...
5. **Patrick-confirm:** verify find@outreach.finda.sale is a verified "send mail as" alias in Gmail settings (live send evidence indicates it is).
