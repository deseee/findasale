...
2. **Hunt Pass copy gap — product decision needed** — hunt-pass.tsx "What's included" lists "Hunt Pass Insider Newsletter" with no backend implementation. Options: remove the benefit, add "Coming soon", or implement. Also: add "Higher XP caps on Treasure Hunt Scans (150/day vs 100/day)" to benefit copy — exists in code, not listed. Dispatch findasale-dev for copy-only change.
3. **Chrome QA backlog — S543 P2 fixes** — After push: verify ActionBar Treasure Trails → /trails; Hunt Pass Active badge gone for Karen on /shopper/dashboard; /shopper/ranks Scout boundary correct.
4. **Continue Chrome QA backlog** — S540 Rewards nav links (4 locations), dashboard achievements dedup, orphan ref hops, S529 storefront widget, #267 RSVP XP, per-sale analytics, settlement fee %.
5. **Organizer Insights runtime error (P2)** — "Failed to load" on /organizer/insights. Pre-existing. Check Railway logs.
6. **S542 hold price + Remove button** — Karen had no active holds during smoke test. Find a shopper with holds to verify $197.81 (not $1.98) and Remove button works in cart drawer.
...
**S543 (2026-04-22, COMPLETE):** Fixes + smoke test + Hunt Pass audit. S542 Chrome-verified: cart drawer ✅, Explore▾ dropdown ✅, ThemeToggle in AvatarDropdown ✅. S541 bug reanalysis: /coupons Generate buttons debunked as VM viewport artifact — fully functional (ss_2443ofv38). #241 Brand Kit PDFs confirmed already fixed in S542 (code review). Fixes dispatched: P0 print kit 500 (getDrafts Decimal serialization — itemController.ts), P2 batch (ActionBar Trails href, HP badge userData→user, RankHeroSection Scout boundary), minor coupons.tsx type improvement. Hunt Pass audit: newsletter benefit is copy-only (no backend — product decision needed); HP XP caps on treasure hunt scans exists in code but missing from hunt-pass.tsx copy. 5 files changed. Push block below.
