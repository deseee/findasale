...
**Latest work (S541 — COMPLETE):** QA-only session. 6 features Chrome-verified, 3 new bugs surfaced. Verified: /coupons Rarity Boost (shopper, 530 XP, ss_4737i417x), /coupons organizer view (no Rarity Boost, ss_56564zsz7), /shopper/loyalty→/coupons redirect (ss_8103rmsqm), /shopper/referrals (referral link REF-0215DAB8, copy + 5 share buttons, ss_59914h5dd), /shopper/appraisals submit (JS `.click()` required — coordinate click is VM viewport artifact), /shopper/early-access-cache (loads correctly). Bugs: (P0) Print kit /organizer/print-kit/cmnxvyic4001li51qobwidrbl returns "Failed to load" — `/api/items/drafts?saleId=cmnxvyic4001li51qobwidrbl` returns 500; sale is RETAIL type with Apr 1–30 (month-long) dates, 87 items in DB — likely RETAIL type or date range triggers bug in items/drafts controller. (P1) #241 Brand Kit PDFs confirmed STILL BROKEN (S531 auth fix incomplete — hrefs hardcoded as `/api/brand-kit/organizer/...` relative to Vercel, no Next.js rewrite exists; fix: change to `${NEXT_PUBLIC_API_URL}/brand-kit/organizer/...`). (P1) /coupons Shopper coupon Generate buttons completely non-functional — no API call fires on click. Deferred P2 findings: ActionBar Treasure Trails href wrong (/shopper/trails → should be /trails), "Hunt Pass Active" badge showing for Karen (no Hunt Pass), /shopper/ranks rank badge/earned message mismatch at Scout boundary. S540 push verified live (Vercel green confirmed by Patrick at session start).
...
- **#241 Brand Kit PDFs (P1):** ❌ STILL BROKEN (S541 Chrome-verified) — S531 auth fix was incomplete. Root cause: brand-kit.tsx hrefs hardcoded `/api/brand-kit/organizer/...` (relative to Vercel). No Next.js rewrite proxies this to Railway. Fix needed: change hrefs to `${process.env.NEXT_PUBLIC_API_URL}/brand-kit/organizer/...`.
- **Print kit items/drafts 500 (P0):** ❌ NEW BUG (S541 Chrome-verified) — `/api/items/drafts?saleId=cmnxvyic4001li51qobwidrbl` returns 500. Sale "Artifact Downtown Paw Paw" is RETAIL type, dates Apr 1–30 (month-long), 87 items in DB, status PUBLISHED, organizer artifactmi@gmail.com. GET /api/sales/{id} returns 200 but items call 500s (fires twice on page load). Likely cause: RETAIL sale type or month-long date range triggers unhandled case in items/drafts controller. Organizer tried to change dates to whole-month range — may have triggered the bug. Investigate itemController getDrafts for RETAIL type handling.
- **/coupons Shopper coupon Generate buttons (P1):** ❌ NEW BUG (S541) — Generate buttons on /coupons shopper section completely non-functional. Clicking produces no API call, no toast, no visual change. Needs dev dispatch to check button onClick wiring and coupon generation endpoint.
...
| Print kit items/drafts 500 (P0) | ❌ NEW BUG S541 | `/api/items/drafts?saleId=cmnxvyic4001li51qobwidrbl` returns 500. RETAIL sale, Apr 1–30 dates, 87 items in DB. Investigate getDrafts controller for RETAIL type / month-long date range handling. | S541 |
| /coupons Shopper coupon Generate buttons (P1) | ❌ NEW BUG S541 | No API call fires on click. Needs onClick wiring + coupon generation endpoint check. | S541 |
...
**S542 priority queue:**
1. **FIRST: Investigate print kit 500 (P0)** — `/api/items/drafts?saleId=cmnxvyic4001li51qobwidrbl` returns 500. Sale "Artifact Downtown Paw Paw" is RETAIL type, Apr 1–30 (whole month). Start: `grep -r "drafts" packages/backend/src/routes/` to find route, then read getDrafts in itemController.ts. Test directly via curl with organizer JWT. Check if RETAIL type or month-long date range triggers unhandled code path. Organizer artificially changed dates to whole-month — may have triggered null/undefined on a date field.
2. **Fix #241 Brand Kit PDFs (P1)** — Change hrefs in `packages/frontend/pages/organizer/brand-kit.tsx` from `/api/brand-kit/organizer/...` (relative Vercel) to `${process.env.NEXT_PUBLIC_API_URL}/brand-kit/organizer/...` (Railway direct).
3. **Fix /coupons coupon Generate buttons (P1)** — No API call fires on click. Read `packages/frontend/pages/coupons.tsx` organizer Generate button onClick, trace to coupon generation endpoint, wire correctly.
4. **P2 fixes (batch):** ActionBar.tsx line 27 `/shopper/trails` → `/trails` (Treasure Trails public browse); "Hunt Pass Active" badge showing for Karen who has no Hunt Pass (find badge source); /shopper/ranks rank badge/earned message mismatch at Scout boundary.
...
**Patrick actions:**
- No pending pushes — S540 is live (Vercel green confirmed S541 start)
...
**S536 (2026-04-21, COMPLETE):** XP economy security audit + hardening + deferred wirings. Hacker agent ran full audit — 19 findings (2 P0, 8 P1, 9 P2). All dispatched in parallel (4 fix agents + 3 wiring agents). Key fixes: cap checks now fail-closed, spendXp atomic via updateMany WHERE guard, REFERRAL_FIRST_PURCHASE gets 24h holdUntil + purchaseId, ORGANIZER_REFERRAL_PURCHASE atomic, both referral XP awards linked to purchaseId, SALE_PUBLISHED one-time milestone, HAUL_POST cap renamed HAUL_POST:60, ORG_HAUL_FROM_SALE capped at 100/month, leaderboard no longer exposes userId, referral codes use crypto.randomBytes. Three deferred wirings shipped: HAUL_POST_LIKES (5 XP), ORG_SHOPPER_SIGNUP (10 XP), REFERRAL_ORG_FIRST_SALE (50 XP). Open gap: phoneVerified not on User model.
