...
1. **All S939 code is pushed and live** (deliverability hardening: gmailHealthCron false-alarm fix, outreachEmailsCron placeholder guard, Resend webhook 4 fixes, soft-bounce policy + migration applied to Railway). RESEND_WEBHOOK_SECRET set in Railway; Resend dashboard webhook created. Nothing pending.
2. **Monitor (no action unless it recurs):** the `@system.finda.sale` bounce DSNs into deseee@gmail.com should now be fully stopped — both the rail-level zone-block (S937d) and the outreachEmailsCron direct-send guard (S939) are live. If NEW `@system` DSNs keep arriving, flag it.
...
**Email/deliverability — RESOLVED this session (no carry-forward):**
- ~~Email P0 E2E test~~ **DONE** — G1 transactional rail was E2E-verified S937f (registration verify email from noreply@finda.sale to inbox); RESEND_FROM_EMAIL=noreply@finda.sale is live.
- ~~RESEND_WEBHOOK_SECRET unset~~ **DONE S939** — set in Railway; webhook signature enforcement E2E-verified (tampered sig → 401).
- ~~Resend bounce/complaint/suppression ingestion broken~~ **DONE S939** — webhook was broken 4 ways (CSRF, raw-body, event-name, payload extraction); all fixed + LIVE e2e-verified (real complaint wrote an EmailSuppression row, hard bounce set bounceHard, delivered reset the counter).
- ~~Soft-bounce one-strike policy~~ **DONE S939** — consecutive threshold (5, reset on delivery) shipped; migration applied.
- ~~Gmail-rail false-alarm P0~~ **DONE S939** — gmailHealthCron now probes via getAccessToken (send-scope only); no re-auth needed.
- ~~Gmail-rail follow-up #1 (SES→GMAIL_FROM_EMAIL split)~~ **DONE S938.**
- ~~Gmail-rail follow-up #2 (bounce ingestion catching 0 rows)~~ **EXPLAINED + RESOLVED S938/S939** — 0 Gmail-inbox rows is correct (only zone-blocked @system DSNs exist); the Resend webhook path now actively ingests real bounces/complaints/suppressions.

**Optional (non-blocking) — Gmail outreach-token re-auth:** the prod Gmail token is send-only (`gmail.send`). Sending works fine and needs no change. Re-auth with a read scope is only required if Patrick ever wants the cron to make read-scope Gmail API calls (e.g. mailbox introspection). Not needed now.

**Remaining Gmail-rail hygiene (lower priority, from gmail-rail-audit-s937.md):**
- **send.finda.sale has no Google DKIM** — latent: if any Gmail fallback ever resolves to @send.finda.sale, sends go unsigned → spam. Gmail fallbacks already point at @outreach.finda.sale (S938); confirm none regress.
- **Patrick-confirm:** find@outreach.finda.sale is a verified "send mail as" alias in Gmail settings (live send evidence indicates it is — S938 smoke test delivered from it).

**Remaining QA items (non-email):**
- **#470 GA4 Conversion Events** — favorite event RUNTIME-VERIFIED S939 (see Pending Chrome Verifications); other 3 events (sale_created/first_item_uploaded/checkout_initiated) CODE-ONLY, verify in GA4 Real-Time on actual action.
- **SEO3 /estate-sales/denver-co** — live-verified S939 (see Pending Chrome Verifications); note: 50 Denver listings all show "Ended" — render correct, data-freshness gap flagged.
...
**DEV candidates:**
- Scraper cron follow-up: verify widened FB Events + PLACES_QUERIES flea synonyms produce new records on next monthly run (S934).
