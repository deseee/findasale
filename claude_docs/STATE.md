...
| CI gate — typecheck BLOCKING; tests+lint still non-blocking | **PARTIALLY RESTORED S1027.** Both typecheck steps now gate (continue-on-error removed); verified 0 errors via faithful fresh-install repro (backend was already clean — S1026 "232" did not reproduce; frontend fixed via tsconfig `types` allowlist). Dockerfile `tsc || true`→`tsc`. | Optional follow-up to fully gate: (a) backend tests — add `--forceExit`/CI flags + test infra so `jest` completes; (b) frontend lint — add an eslint config (`eslint-config-next`) + clean violations, then flip those two `continue-on-error` to false. Not blocking; typecheck is the real protection. | S1026 → partial S1027 |
