...
**Latest work (S547 — COMPLETE):** Mobile overflow fix on organizer dashboard past sales cards. Both "Past Sales" blocks in `packages/frontend/pages/organizer/dashboard.tsx` (State 2 list at line 1520, State 3 archive at line 1640) used a horizontal flex where title+city (`flex-1`) sat next to three inline action items (Reopen button, Settle link, View Details link) — on narrow mobile viewports the buttons pushed past the card edge. Fix: made outer row responsive (`flex-col sm:flex-row sm:items-center sm:justify-between gap-*`), wrapped the three actions in a `flex flex-wrap items-center gap-2` container so they wrap under the title on mobile and sit right-aligned on desktop, added `min-w-0` + `truncate` to the title/city so long sale names don't blow out layout. Zero logic changes — Tailwind class edits only, same three action elements preserved. 1 file.
...
**S548 priority queue:**
0. **Mandatory live smoke test per CLAUDE.md §10** — S547 past sales card mobile overflow fix. Open /organizer/dashboard on 375px viewport as organizer with ENDED sales present; confirm title/buttons stack cleanly and desktop layout is unchanged.

**Carry-over from S547 queue:**
1. **Mandatory live smoke test per CLAUDE.md §10** — Verify S545 + S546 changes work end-to-end in Chrome as real users:
   - Sale detail page `/sales/[id]` — rank lock behavior for INITIATE shopper (presale window should show 🔒 + Rank-Up CTA); for Scout/Ranger/Sage/GM the page should unlock per the tier windows (1h/2h/4h/6h)
   - SaleLockCard renders correctly with correct copy + rank icons
   - Affiliate Program backend — `GET /api/affiliate/me` returns user's referralCode, referrals list, totals (no UI yet — this is Batch 1 foundation)
   - S545 mobile organizer dashboard fix (dashboard.tsx flex classes)
   - S545 Organizer Insights Decimal fix for Alice
2. **Chrome QA backlog from S543/S540/S541** — ActionBar Trails href, HP badge userData→user, RankHeroSection Scout boundary, guild-primer perks, Rewards nav (4 locations), dashboard achievements dedup, #267 RSVP XP, per-sale analytics, settlement fee %.
3. **Affiliate Program Batches 2–10** — Architect spec at `claude_docs/feature-notes/affiliate-program-spec-S544.md`. Patrick decisions still open on final payout amounts (placeholder config $20/$55). Organizer-only scope confirmed. Fraud thresholds: 7-day account age, 30-day lockout.
4. **S542 hold price + Remove button** — still needs shopper with active holds to verify $197.81 (not $1.98).
