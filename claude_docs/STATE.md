...
**S813 complete:** #424/#425/#426 eBay QA all verified. Map pins root cause found + fixed (discoveryService.ts — authenticated users bypassed regional bounding box). Pushed green.
...
2. **GitGuardian API token:** Create a GitGuardian personal access token (dashboard.gitguardian.com → API → Personal access tokens) with `incidents:read` scope.
...
5. **Map pins smoke test:** After Railway deploys discoveryService.ts fix, reload finda.sale while logged in — pins should now appear near Grand Rapids/Paw Paw.

**Dispatch stubs for next session:**
- Main session Chrome QA → **Map pins smoke test** (after Railway deploys): log in as Artifact MI, load finda.sale homepage, confirm pins appear near GR/Paw Paw region.
